...
def get_auth_service(
    ...
    return AuthService(UserRepository(session), captcha_service, get_settings())
...
def register(
    ...
    user = service.register(**request.model_dump())
...
def login(request: LoginRequest, service: AuthServiceDependency) -> ApiResponse[TokenData]:
    user, token, expires_in = service.login(**request.model_dump())
    data = TokenData(
        access_token=token,
