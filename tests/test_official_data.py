...
def test_official_html_cleaning_removes_scripts_and_duplicate_lines() -> None:
    html = "<script>bad()</script><h1>政策标题</h1><p>正文</p><p>正文</p>"
    text = extract_text(html)
    assert "bad" not in text
    assert text.splitlines() == ["政策标题", "正文"]
...
def test_evaluation_seed_has_50_unique_complete_records() -> None:
    ...
    assert {item["risk_level"] for item in records} >= {"LOW", "MEDIUM", "HIGH", "PROHIBITED"}
